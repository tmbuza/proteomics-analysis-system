...
#
# Usage:
#   Rscript scripts/R/05-filter-differential-proteins.R data/example/example-proteomics-results.csv results 1 0.05
...
if (length(args) < 1) {
  stop(
    paste(
      "Usage:",
      "Rscript scripts/R/05-filter-differential-proteins.R <input_table> [output_dir] [abs_log2fc_threshold] [adjusted_p_value_threshold]",
      sep = "\n"
    ),
    call. = FALSE
  )
...
cat("Differential protein abundance filtering complete.\n")
cat("\n")
